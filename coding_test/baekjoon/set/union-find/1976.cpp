...
Method :
최단 경로와 같이 특별한 조건이 없고, 단순히 돌아서라도 경로에 도착할 수 있으면 YES를 출력한다.
즉 경로에 있는 모든 노드들이 어떤식으로든 연결이 되어 있다면 도착할 수 있는 것으로 판단한다.
따라서 union-find 알고리즘을 통해 입력으로 들어오는 연결 상태에 따라서 집합을 합치면서,
마지막의 m개의 경로에 대해 모두 같은 집합인지를 확인해서 같다면 YES, 하나라도 틀리다면 NO를 출력한다.
Example :
3
3
0 1 0
