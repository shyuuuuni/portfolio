...
최단 경로를 출력한다.
Method :
dist 배열을 다음과 같이 선언한다.
dist[is_broken][x][y] : (x,y)로 도달하기 위해 지나온 칸 수
(이때 is_broken은 그동안 벽을 부셨다면 1, 아니라면 0)
초기 BFS는 출발점 (0,0)에서 시작하므로 ((0,0),0) 을 입력한다.
매 BFS 시행마다 만약 is_broken이라면 벽을 통과할 수 없고,
벽이 아니여도 더 빠른 방법이 있다면 큐에 추가하지 않는다.
is_broken이 아니라면 벽이 아니라면 BFS로 이동하면 되고,
만약 이동하는 위치가 벽이라면 부수고 큐에 is_broken을 1로 설정한다.
*/
#include <bits/stdc++.h>
#define X first
#define Y second
