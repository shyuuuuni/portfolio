...
Method :
BFS를 여러번 돌아서 해결하는 풀이로 접근하였다.
배열에 어떤 좌표에 어느곳의 불을 켤 수 있는지를 저장한뒤,
처음에 (1,1)에서부터 BFS를 돌면서, 만약 켜지 않은 스위치가 있다면 flag=1로 설정하고 스위치를 켜 준다.
flag=1이면, 다음 BFS를 계속 돈다는 의미이므로, 현재 시행으로 킬 수 있는 스위치를 다 키고, 그 다음 스위치를 킨다.
flag=0이면, BFS를 모두 돌았는데, 하나의 스위치도 키지 못했다는 의미이므로 더이상 탐색할 수 없으므로 반복을 종료한다.
마지막으로 완전탐색으로 켜져있는 방의 개수를 출력한다.
Example :
3 6
1 1 1 2
2 1 2 2
