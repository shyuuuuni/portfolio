...
Method :
BFS에 jump 옵션을 추가하여 해결하였다.
tile 배열은 말 그대로 격자판에 0과 1을 담는 배열이다.
dist[jump][x][y] : 좌표(x,y)로 나이트 이동을 jump번 이용하여 도달하는데 걸린 시간이다.
만약 jump값이 k보다 작다면 아직 나이트 이동을 할 수 있으므로,
탐색시 jump+1로 변경하면서 나이트 이동을 추가하였다.
*/
#include <bits/stdc++.h>
#define X first
#define Y second
