...
Method :
d[i][j]를 i번째 줄에 있는 j번째 위치까지 가는데 필요한 최소 비용으로 정의한다.
초기값으로 (0,1)에서 출발하므로 d[0][1] = a[0][1]이고, 그 왼쪽으로 가는 방법은 없으니 큰 수를 넣어주고,
오른쪽으로 가는 방법은 d[0][2] = a[0][1] + a[0][2]이다.
이후부터는 그림에 나와있는데로 d[i][0,1,2]를 각각 최소치로 갱신하면 된다.
Example :
4
13 7 5
7 13 6
