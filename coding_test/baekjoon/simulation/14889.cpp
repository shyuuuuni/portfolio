...
Algorithm Type : Simulation, BackTracking
Outline :
...
Method :
N의 값이 충분히 작기 때문에 팀을 나누는 모든 경우의 수를 생각해도 시간 초과가 발생하지 않는다.
따라서 최대 20명 중에 10명을 고르면 나머지는 또 하나의 팀이 되므로 20C10 으로 완전 탐색이 가능하다.
조합이므로, STL의 next_permutation을 이용하여 구현하였고,
a[i] == 1이면 스타트팀, a[i] == 0이면 링크팀으로 생각하고,
i=0 ~ n-1 까지 돌면서, a[i] == 1 이면 나머지 a[j] == 1 인 모든 j에 대해서 start += s[i][j]를 해주었다.
(이렇게 하면 나중 루프에서 s[i][j] 뿐만 아니라 s[j][i]를 더해준다.)
Example :
4
0 1 2 3
4 0 5 6
