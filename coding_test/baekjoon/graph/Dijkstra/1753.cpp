...
Method :
출발지에서 모든 정점으로의 최단 거리를 구하는 문제이므로 간단하게 다익스트라 알고리즘으로 해결한다.
그러나 이 경우 정점의 개수가 20000까지 굉장히 크기 때문에,
일반적인 다익스트라 방법을 사용하면 O(V^2+E) 로 터질 수 있기 때문에
우선순위 큐를 이용한 O(ElgE) 방법을 사용하여 해결한다.
Example :
5 6
1
5 1 1
