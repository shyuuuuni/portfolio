...
Method :
목적지가 여러개일 뿐이지, 사실상 꺼꾸로 가는 다익스트라 문제와 일치한다.
간단하게 생각해서 출발지 하나에서 가중치가 0인 간선으로 각각의 면접장 모두에 이어져 있다고 생각하면 될듯하다.
그래서 pq에 면접장들을 거리 0으로 두고,s
invadj[i] : i번째 도시로 "들어오는" 간선으로 저장해서,
역으로 다익스트라를 처리하면 된다.
이때 도시의 개수가 많고 가중치가 꽤 커서 long long으로 처리하지 않았을 때 틀렸습니다가 떴다. 이를 주의하자.
Example :
6 10 2
2 6 2
1 4 1
