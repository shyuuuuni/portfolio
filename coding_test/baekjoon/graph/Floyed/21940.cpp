...
만족하는 도시 번호를 출력한다.
Method :
결국 각 노드에서 다른 노드로의 최단 거리를 구한 후,
K명의 사람 각각의 출발지 k로부터 dist[k][X] + dist[X][k] 값의 최댓값이 최소가 되는 X를 선택하는 문제이다.
따라서 플로이드 알고리즘으로 먼저 최단 거리를 구한 후,
for문을 통해 해당 왕복시간의 최댓값의 최솟값을 구한다.
Example :
4 9
1 2 9
2 3 9
