...
Method :
백트래킹으로 현재 계란으로 칠 수 있는 모든 경우를 모투 탐색하였다.
dfs(idx, bk) 함수는 현재까지 bk개의 계란을 깼을 때, idx번째 계란을 들고 다른 계란을 깨는 경우를 의미한다.
먼저 들고있는 idx번째 계란이 깨졌다면, idx+1로 넘어간다.
이후, 1부터 n까지의 계란에 대해서, idx가 아니면서 깨지지 않은 계란이 있다면, 해당 계란을 깨는 상황을 탐색한다.
dfs함수에서 idx가 n보다 크게 된다면 종료 조건으로 ans와 bk를 비교해서 최댓값으로 갱신한다.
만약 1부터 n까지의 계란에 대해서 모두 탐색할 수 없다면 바로 dfs(n+1,bk)를 호출해서 종료한다.
Example:
3
8 5
1 100
