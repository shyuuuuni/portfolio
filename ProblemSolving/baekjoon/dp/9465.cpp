...
d[i][j] 를 다음과 같이 정의한다.
"j번째 column까지 봤을 때, 
d[0][j] : 0번째 줄을 선택했을 때 최댓값
d[1][j] : 1번째 줄을 선택했을 때 최댓값
d[2][j] : 해당 column에서 선택하지 않았을 때 최댓값"
그러면 d[0][j]는 0번째 줄을 선택하므로, 이전 줄에서는 선택하지 않았거나 1번째 줄을 선택해야 하고,
d[1][j]는 선택X or 0번째 줄 선택
d[2][j]는 d[0][j-1] or d[1][j-1] 중에 선택하면 된다.
Example :
2
5
50 10 100 20 40
