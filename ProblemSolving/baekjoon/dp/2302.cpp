...
Method :
d[i][j]를 다음과 같이 정의한다.
"왼쪽에서부터 차례대로 봤을 때, i번째 좌석을 j=0이면 i번째 사람이 앉는 경우, j=1이면 i번째 사람이 앉지 않는 경우" 로 정의한다.
d[i][j]는 그러면 다음과 같은 점화식으로 표현된다.
d[i][0] = d[i-1][0] + d[i-1][1] // i번째 사람이 그 자리에 앉으면, 그 이전의 경우의 수 개수만큼 존재한다.
d[i][1]
  만약 i번째가 VIP 좌석이라면 0
  만약 i-1번째가 VIP 좌석이라면 0
  그렇지 않다면 d[i-1][0] // i-1번째가 그 자리에 앉아야 i번째와 자리를 옮기는 경우가 생김
Example :
9
2
4
