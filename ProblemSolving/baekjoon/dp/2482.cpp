...
Method :
d[x][y][k][n] 을 다음과 같이 정의한다.
x : n번째 수를 선택했는지 여부 (0 : 선택X, 1 : 선택O)
y : 1번째 수를 선택했는지 여부 (0 : 선택X, 1 : 선택O)
k : n번째 수까지 봤을 때 선택한 수의 개수
n : 1~n번째 수까지 봄
여기서 y는 색상환이 원이기 때문에 배열같이 생각하다가도 첫번째 색과 마지막 색이 붙어있기 때문에 이를 처리하기 위함이다.
y가 0인것은 0인것끼리, 1인것은 1인것끼리 계산한다.
d[0][y][k][n] = d[1][y][k][n-1] + d[0][y][k][n-1] : 이번 것을 선택하지 않았을 때 합이 k인 경우
d[1][y][k][n] = d[0][y][k-1][n-1] : 이번 것을 선택하므로 이전 것은 선택하면 안된다.
또한 n이 마지막 원소일 때 d[1][1][k][n] 값은 앞서 설명한대로 불가능하므로 0이다.
Example :
4 2
-------
2
