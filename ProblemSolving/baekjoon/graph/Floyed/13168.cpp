...
Method :
도시의 이름만 인덱스로 변환하면, 여행 순서대로 최단 거리로 이동하는 플로이드 와샬 문제와 같다.
따라서 STL의 맵을 이용하여 들어오는 string을 int로 변환시켜준다.
또한 내일로 티켓을 구매했을 때, S-Train, V-Train은 반값으로 할인해주는데, 이 경우 홀수가 들어올 경우에는 실수 범위 계산이 들어간다.
따라서 이를 방지하기 위해 모든 가격을 두배를 해주어 짝수라는 전제를 깔아주고 계산한다.
Example :
14 40000
