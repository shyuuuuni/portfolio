...
Method :
플로이드를 업데이트할때 거쳐가는 방법의 합으로 업데이트 하는 것이 아니라 거쳐갔을 때 최대 높이를 기준으로 업데이트하면 끝나는 문제이다.
1-ㄴ>5로 직선거리일 때 높이가 10인 허들을 지나야 하는데, 1->2가 높이가 3, 2->3이 높이가 5라면 1->2->3이면 max(3,5) = 5로 업데이트 가능하다는 뜻.
Example :
5 6 3
1 2 12
3 2 8
