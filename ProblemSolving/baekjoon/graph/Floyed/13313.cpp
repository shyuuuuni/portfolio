...
Method :
먼저 주어진 코드에서 틀린 부분은, 플로이드 알고리즘에서 중간에 거쳐서 가는 정점인 K를 체크하지 않는다는 점이다.
즉, K번째 정점을 거쳐서 최단 거리가 갱신되는 경우 정상 알고리즘과 결과가 차이날 수 밖에 없다.
따라서 i==j일때를 0으로 두고, k를 거치는 것이 최단 경로가 되도록, i -> k인 간선을 가중치를 0으로 두고,
k -> i인 간선의 가중치를 모두 0으로 두면 항상 i -> k -> j가 0으로 최단거리가 된다.
틀린 코드는 해당 부분을 탐색하지 않으므로 최단 거리와 차이가 생길 수 밖에 없다.
따라서 N=100일때로 처리를 하면 9700개 이상의 차이가 발생하게 된다.
Example :
입력은 없다.
-------
첫번째 줄에 N(정점의 개수)
