...
Method :
결국 1번에서 시작해서 (A,B)를 찍고 N으로 이동하므로
1->A->B->N or 1->B->A->N 의 최솟값을 구하면 된다.
따라서 1에서 출발하는 최단경로, A에서 출발하는 최단경로, B에서 출발하는 최단경로를 각각 다익스트라로 계산하고,
그 최솟값을 구하면 된다.
마지막에 INF + @가 될 수 있으므로 불가능 조건을 INF <= ans 일 경우 불가능으로 판단한다.
Example :
4 6
1 2 3
2 3 3
