...
Method :
왼쪽->오른쪽 / 위->아래로 이동하는 경우로 생각한다. (사실 같은 방법이고, row/col만 바꾸면 된다)
왼쪽->오른쪽으로 이동하면서 현재 위치 x 기준으로 경사로를 놓는 두 가지 방법을 생각한다.
1. x~x+l-1까지 경사로를 놓는다 : 즉, x, x+1, ..., x+l-1 까지의 높이가 h면, x+l의 높이가 h+1인 경우
2. x+1~x+l까지 경사로를 놓는다 : 즉, x+1, x+2, ..., x+l 까지의 높이가 h면, 현재 x의 높이가 h+1인 경우
1번의 경우 x->x+l 방향으로 올라가는 경사로, 2번의 경우 x+1->x+l 방향으로 내려가는 경사로를 놓는다.
(항상 out of bound 범위와, 경사로를 놓는 구간의 값이 같은 값인지는 확인한다.)
이렇게 처리한 후 재귀적으로 checkrow 함수에서 다음 위치로 x+l로 이동한다.
그러나 2번의 경우 다음 checkrow 함수의 x가 x+l이 되는데, 해당 위치에 이미 경사로가 놓였기 때문에 놓을 수 없다.
이 부분을 is_jump라는 변수로 처리했고, is_jump가 1이면 1번의 경우 현재 위치에 경사로를 놓기 때문에 놓을 수 없는걸 처리해주어야한다.
같은 방법으로 checkcol 함수를 구현해서 올바른 길의 개수를 세어준다.
Example :
6 2
3 3 3 3 3 3
2 3 3 3 3 3
