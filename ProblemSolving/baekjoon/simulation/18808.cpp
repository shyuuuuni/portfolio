...
Method :
먼저 스티커를 90도 회전시키는 swing() 함수를 구현한다.
(x,y)좌표의 값을 swing() 하면 (y,sticker_y - 1 - x) 위치로 이동함을 이용하여 구현한다.
또한 스티커를 (x,y)좌표를 시작으로 붙일 수 있는지 검사하는 check 함수를 구현한다.
단순하게 (x,y) 좌표에서 스티커를 붙였을 때 좌표를 검사하면서 한군데라도 틀린 부분이 있다면
false를 리턴하도록 하고, 모두 괜찮다면 해당 위치에 스티커를 붙이고 true를 리턴한다.
즉, 매 스티커마다 0도,90도,180도,270도 회전한 스티커를 모든 좌표에 대해서 붙일 수 있는지 확인한다.
이렇게 모든 스티커의 경우를 처리하고 마지막에 비어있는 부분의 개수를 출력한다.
*/
#include <bits/stdc++.h>
#define X first
#define Y second
