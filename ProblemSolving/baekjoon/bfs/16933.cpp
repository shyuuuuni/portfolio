...
또한 낮에만 벽을 부술 수 있다.
Method :
이동 거리를 저장하는 배열을 낮, 밤 따로 정한다.
dist_noon[i][j][k]는 벽을 k번 부셔서 낮에 (i,j)로 도착하는 최단 경로,
dist_night[i][j][k]는 벽을 k번 부셔서 밤에 (i,j)로 도착하는 최단 경로를 저장한다.
매 BFS 시행마다 낮->밤, 밤->낮이 바뀐다.
또한 기존에는 BFS시 a[i][j] == 1 (벽) 이라면 진행하지 않았는데,
만약 제자리에 서 있는 경우 처리를 해 주어야 하기 때문에,
nx == x && ny == y 인 경우로 따로 처리를 해 주었다.
이외에는 이전 벽부수기 문제와 비슷하므로 생략한다.
Example :
1 4 1
0010
-------
