...
최단 경로를 출력한다.
Method :
dist[i][j][k]를 벽을 k번 부수고 이동했을 때, (i,j)에 도착하는 최단 경로로 정의한다.
dist배열은 모두 mx값 (1000000+5)로 초기화해둔다.
BFS를 돌면서 만약 다음에 이동할 위치 (nx,ny)가 벽일 경우와 벽이 아닐 경우로 나뉜다.
1. 벽일 경우 탐색하려면 무조건 벽을 부셔야 하므로, 만약 현재까지 부순 벽이 k일 경우 더이상 탐색이 불가능하다.
   그렇지 않다면 다음 위치는 dist[nx][ny][bk+1]로 탐색해야 하며, 현재 (x,y)에서 이동하는 경우가 최소인지 탐색한다.
2. 벽이 아닌 경우 일반적인 BFS처럼 현재 이동하는 것이 최소일 경우 dist[nx][ny][bk]를 갱신해준다.
마지막으로 dist[n-1][m-1][0~k] 값의 최솟값을 구하고, 그 값이 mx가 아닌 경우 출력한다.
Example :
6 4 1
0100
1110
