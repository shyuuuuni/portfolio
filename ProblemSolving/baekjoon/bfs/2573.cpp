...
Method :
BFS를 이용하여 해결하였다.
queue에 높이가 0인 부분의 좌표(x,y)와 해당 시점의 년도 year를 입력한다.
y_cnt는 매 년도가 처음 나올때마다 갈라졋는지를 체크하기 위한 변수이다.
year == y_cnt일 경우 높이가 가장 높은 빙산에서부터 BFS를 돌려서
cnt == n_mount(현재 남아있는 빙산의 개수) 인지 확인한다.
만약 cnt==0 이거나 cnt != n_mount 일 경우 0(모두 녹아 없어짐)이다.
같은 경우에는 아직 갈라지지 않았으므로 계속 이어나간다.
0은 상하좌우를 보면서 만약 높이가 있는 빙산이라면 높이를 1 줄인다.
만약 1 줄여서 높이가 0이라면 n_mount를 줄이면서
0이 되었으므로 큐에 추가해서 계속 돌린다.
*/
#include <bits/stdc++.h>
#define X first
#define Y second
