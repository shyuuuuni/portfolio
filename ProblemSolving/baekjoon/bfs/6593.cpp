...
Method :
3차원 BFS로 해결한다.
상범이의 초기 위치의 dist(거리) 를 0으로 초기화 하고,
해당 위치에서부터 BFS를 돌리면서 이동할 수 있는 위치일 경우
해당 위치의 dist를 이전 위치의 dist+1로 바꿔주면서 이동한다.
만약 해당 위치가 출구라면 해당 위치의 dist를 ans에 저장하고,
만약 모든 처리를 끝냈는데 ans값이 -1에서 변하지 않았다면 탈출 불가능으로 판단한다.
*/
#include <bits/stdc++.h>
#define X first
#define Y second
